(() => {
  ...
  form.addEventListener('submit',async event=>{
    ...
    const token=++turn,profile=identity(),alive=()=>token===turn&&valid()&&identity()===profile;
    fields.disabled=true;status.textContent='Preparando las 24 pestañas…';
    try{
  ...
  download.addEventListener('click',async()=>{
    ...
    try{
      ...
      status.textContent='Descarga iniciada. El archivo contiene las 24 pestañas y los registros autorizados del alcance elegido.';
    }catch(error){if(alive())status.textContent=error.message;}
  ...
  for(const id of ['consultaRecordsView']){
    ...
    const button=document.createElement('button');button.type='button';button.className='primary';button.textContent='⇩ Excel completo · 24 pestañas';button.addEventListener('click',open);toolbar.append(button);
  }
