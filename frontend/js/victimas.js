(() => {
  ...
  function open(type,record=null,view=false){
    ...
    const historical=window.VICTIMAS_CATALOGO.camposHistoricos.filter(f=>record?.datos[f.key]!=null&&record.datos[f.key]!=='');
  ...
  form.addEventListener('submit',async event=>{
    ...
    datos.condicion_edad=datos.edad<18?'MENOR':'MAYOR';
