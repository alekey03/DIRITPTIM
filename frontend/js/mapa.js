(function initializeCrimeMaps() {
  const urls = { department:'frontend/assets/maps/departamentos.geojson', province:'frontend/assets/maps/provincias.geojson', district:'frontend/assets/maps/distritos.geojson' };
  ...
  function ensure(id) {
    ...
    const hover=document.createElement('div'); hover.className='crime-map-hover'; hover.innerHTML='<span></span><strong></strong><b></b>'; document.getElementById(id).appendChild(hover);
    const instance={map,hover,layer:null,records:[],level:'department',department:null,province:null,label:'casos',animating:false}; instances.set(id,instance); return instance;
  }
  function showHover(instance,event,info,amount) {
    const box=instance.map.getContainer().getBoundingClientRect(); const tooltip=instance.hover; const x=event.clientX-box.left; const y=event.clientY-box.top;
    tooltip.querySelector('span').textContent=instance.level==='department'?'Departamento':instance.level==='province'?'Provincia':'Distrito';
    tooltip.querySelector('strong').textContent=title(info.name); tooltip.querySelector('b').textContent=`${amount.toLocaleString('es-PE')} ${instance.label}`;
    tooltip.style.left=`${Math.max(10,Math.min(x+16,box.width-205))}px`; tooltip.style.top=`${Math.max(10,y-18)}px`; tooltip.classList.add('visible');
  }
  function hideHover(instance){instance.hover.classList.remove('visible');}
  ...
  function enterZone(id, instance, layer, info, amount, event) {
    if(instance.animating)return;
    if(instance.level==='district'){showHover(instance,event,info,amount);return;}
    instance.animating=true; hideHover(instance); layer.setStyle({weight:4,color:'#075c39',fillOpacity:1});
  ...
  async function draw(id) {
    ...
    instance.layer=L.geoJSON({type:'FeatureCollection',features},{style:feature=>({color:'#b88718',weight:1.25,fillColor:color(totals[featureKey(feature,instance.level)]||0,maximum),fillOpacity:.9}),onEachFeature:(feature,layer)=>{
      ...
      layer.once('add',()=>{
        ...
        shape.addEventListener('pointerenter',event=>{layer.setStyle({weight:3,color:'#0f7048',fillOpacity:1});showHover(instance,event,info,amount);});
        shape.addEventListener('pointermove',event=>showHover(instance,event,info,amount));
        shape.addEventListener('pointerleave',()=>{instance.layer?.resetStyle(layer);hideHover(instance);});
        shape.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();enterZone(id,instance,layer,info,amount,event);});
      });
