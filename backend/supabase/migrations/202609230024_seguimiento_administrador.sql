-- El administrador general mantiene acceso completo.
begin;
create or replace function public.consultar_seguimiento(p_fecha date) returns jsonb
language plpgsql stable security definer set search_path='' as $$
declare respuesta jsonb;
begin
 if not exists(select 1 from public.perfiles where id=auth.uid() and activo and rol::text in ('administrador','estadistico_direccion','estadistico_jefatura')) then raise exception 'Seguimiento está reservado para Administración, Dirección y Jefatura.' using errcode='42501';end if;
 ...
 with unidades as (
  ...
  select distinct e.unidad,'OTROS','OTROS',false from public.seguimiento_ingresos e
   ...
   and exists(select 1 from public.perfiles p where p.id=auth.uid() and p.activo and p.rol::text in ('administrador','estadistico_direccion'))
