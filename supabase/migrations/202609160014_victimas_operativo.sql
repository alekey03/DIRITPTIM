-- Hoja 21_VICTIMAS_DE_TRATA: registros independientes, con datos comunes del operativo.
begin;
create table if not exists public.intervencion_victimas (
 id uuid primary key default gen_random_uuid(),
 intervencion_id uuid not null references public.intervenciones(id) on delete restrict,
 tipo text not null check(tipo in ('victima')),
...
create index if not exists victimas_operativo_idx on public.intervencion_victimas(intervencion_id,creado_en,id);
alter table public.intervencion_victimas enable row level security;
drop policy if exists victimas_lectura on public.intervencion_victimas;
create policy victimas_lectura on public.intervencion_victimas for select to authenticated using(exists(select 1 from public.intervenciones i where i.id=intervencion_id));
drop policy if exists victimas_alta on public.intervencion_victimas;
create policy victimas_alta on public.intervencion_victimas for insert to authenticated with check(creado_por=auth.uid() and public.usuario_activo() and exists(select 1 from public.intervenciones i where i.id=intervencion_id and (public.es_administrador() or (i.creado_por=auth.uid() and i.unidad=public.unidad_actual()))));
drop policy if exists victimas_edicion on public.intervencion_victimas;
create policy victimas_edicion on public.intervencion_victimas for update to authenticated using(public.usuario_activo() and exists(select 1 from public.intervenciones i where i.id=intervencion_id and (public.es_administrador() or (i.creado_por=auth.uid() and i.unidad=public.unidad_actual())))) with check(public.usuario_activo() and exists(select 1 from public.intervenciones i where i.id=intervencion_id and (public.es_administrador() or (i.creado_por=auth.uid() and i.unidad=public.unidad_actual()))));
revoke all on public.intervencion_victimas from public,anon,authenticated;
grant select,insert,update on public.intervencion_victimas to authenticated;
...
begin
 ...
 if nullif(btrim(new.datos->>'apellido_paterno'),'') is null or nullif(btrim(new.datos->>'nombres'),'') is null or new.datos->>'edad' is null or new.datos->>'fecha' is null or new.datos->>'situacion' is null then raise exception 'Complete apellidos, nombres, edad, fecha y situación.' using errcode='22023';end if;
...
drop trigger if exists proteger_victima_operativo on public.intervencion_victimas;
create trigger proteger_victima_operativo before insert or update on public.intervencion_victimas for each row execute function public.proteger_victima_operativo();
create or replace function public.conservar_victimas_registrados() returns trigger language plpgsql security invoker set search_path='' as $$
begin
 if exists(select 1 from public.intervencion_victimas where intervencion_id=old.id) and (new.tipo='directa' or not('victimas'=any(new.resultados_previstos))) then raise exception 'El operativo ya tiene víctimas registradas.' using errcode='23514';end if;
 return new;
end $$;
drop trigger if exists conservar_victimas_registrados on public.intervenciones;
create trigger conservar_victimas_registrados before update of tipo,resultados_previstos on public.intervenciones for each row execute function public.conservar_victimas_registrados();
create or replace function public.guardar_victima_operativo(p_id uuid,p_intervencion uuid,p_version integer,p_tipo text,p_datos jsonb) returns jsonb language plpgsql security invoker set search_path='' as $$
declare i public.intervenciones%rowtype; m public.intervencion_victimas%rowtype;
begin
 ...
 select * into m from public.intervencion_victimas where id=p_id for update;
 if found then
  ...
  update public.intervencion_victimas set datos=p_datos where id=p_id returning * into m;
 else
  if p_version<>0 then raise exception 'Registro no disponible.' using errcode='40001';end if;
  insert into public.intervencion_victimas(id,intervencion_id,tipo,datos) values(p_id,p_intervencion,p_tipo,p_datos) returning * into m;
 end if;
...
revoke all on function public.proteger_victima_operativo(),public.conservar_victimas_registrados(),public.guardar_victima_operativo(uuid,uuid,integer,text,jsonb) from public;
grant execute on function public.guardar_victima_operativo(uuid,uuid,integer,text,jsonb) to authenticated;
