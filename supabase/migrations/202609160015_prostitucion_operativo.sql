-- Hoja 20_PROSTITUCION FEM Y MASC: registros independientes, con datos comunes del operativo.
begin;
...
alter table public.intervenciones add constraint resultados_previstos_validos check (
 resultados_previstos <@ array['detenidos','requisitoriados','menores','victimas','prostitucion','bandas','organizaciones','armas','drogas','vehiculos','personas_ubicadas','otros']::text[]
 and array_position(resultados_previstos,null) is null
);
create table if not exists public.intervencion_prostitucion (
 id uuid primary key default gen_random_uuid(),
 intervencion_id uuid not null references public.intervenciones(id) on delete restrict,
 tipo text not null check(tipo in ('prostitucion')),
...
create index if not exists prostitucion_operativo_idx on public.intervencion_prostitucion(intervencion_id,creado_en,id);
alter table public.intervencion_prostitucion enable row level security;
drop policy if exists prostitucion_lectura on public.intervencion_prostitucion;
create policy prostitucion_lectura on public.intervencion_prostitucion for select to authenticated using(exists(select 1 from public.intervenciones i where i.id=intervencion_id));
drop policy if exists prostitucion_alta on public.intervencion_prostitucion;
create policy prostitucion_alta on public.intervencion_prostitucion for insert to authenticated with check(creado_por=auth.uid() and public.usuario_activo() and exists(select 1 from public.intervenciones i where i.id=intervencion_id and (public.es_administrador() or (i.creado_por=auth.uid() and i.unidad=public.unidad_actual()))));
drop policy if exists prostitucion_edicion on public.intervencion_prostitucion;
create policy prostitucion_edicion on public.intervencion_prostitucion for update to authenticated using(public.usuario_activo() and exists(select 1 from public.intervenciones i where i.id=intervencion_id and (public.es_administrador() or (i.creado_por=auth.uid() and i.unidad=public.unidad_actual())))) with check(public.usuario_activo() and exists(select 1 from public.intervenciones i where i.id=intervencion_id and (public.es_administrador() or (i.creado_por=auth.uid() and i.unidad=public.unidad_actual()))));
revoke all on public.intervencion_prostitucion from public,anon,authenticated;
grant select,insert,update on public.intervencion_prostitucion to authenticated;
...
begin
 ...
 if not found or i.tipo not in ('operativo','megaoperativo') or not public.usuario_activo() or not(public.es_administrador() or (i.creado_por=auth.uid() and i.unidad=public.unidad_actual())) then raise exception 'No puede modificar registros de prostitución de este operativo.' using errcode='42501'; end if;
...
drop trigger if exists proteger_prostitucion_operativo on public.intervencion_prostitucion;
create trigger proteger_prostitucion_operativo before insert or update on public.intervencion_prostitucion for each row execute function public.proteger_prostitucion_operativo();
create or replace function public.conservar_prostitucion_registrados() returns trigger language plpgsql security invoker set search_path='' as $$
begin
 if exists(select 1 from public.intervencion_prostitucion where intervencion_id=old.id) and (new.tipo='directa' or not('prostitucion'=any(new.resultados_previstos))) then raise exception 'El operativo ya tiene registros de prostitución.' using errcode='23514';end if;
 return new;
end $$;
drop trigger if exists conservar_prostitucion_registrados on public.intervenciones;
create trigger conservar_prostitucion_registrados before update of tipo,resultados_previstos on public.intervenciones for each row execute function public.conservar_prostitucion_registrados();
create or replace function public.guardar_prostitucion_operativo(p_id uuid,p_intervencion uuid,p_version integer,p_tipo text,p_datos jsonb) returns jsonb language plpgsql security invoker set search_path='' as $$
declare i public.intervenciones%rowtype; m public.intervencion_prostitucion%rowtype;
begin
 ...
 select * into m from public.intervencion_prostitucion where id=p_id for update;
 if found then
  ...
  update public.intervencion_prostitucion set datos=p_datos where id=p_id returning * into m;
 else
  if p_version<>0 then raise exception 'Registro no disponible.' using errcode='40001';end if;
  insert into public.intervencion_prostitucion(id,intervencion_id,tipo,datos) values(p_id,p_intervencion,p_tipo,p_datos) returning * into m;
 end if;
...
revoke all on function public.proteger_prostitucion_operativo(),public.conservar_prostitucion_registrados(),public.guardar_prostitucion_operativo(uuid,uuid,integer,text,jsonb) from public;
grant execute on function public.guardar_prostitucion_operativo(uuid,uuid,integer,text,jsonb) to authenticated;
