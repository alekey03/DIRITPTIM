(() => {
  ...
  function trend(f){
    ...
    for(let i=0;i<=3;i++){const yy=190-i*50;svg.append(node('line',{x1:55,x2:725,y1:yy,y2:yy,stroke:'#e8eef2'}),node('text',{x:44,y:yy+4,'text-anchor':'end',fill:'#718494','font-size':11},fmt(max*i/3)));}
    const points=items.map((d,i)=>`${x(i)},${y(d.count)}`).join(' ');
    svg.append(node('polygon',{points:`${x(0)},190 ${points} ${x(items.length-1)},190`,fill:'#dceef3',opacity:'.7'}),node('polyline',{points,fill:'none',stroke:'#297f96','stroke-width':3,'stroke-linejoin':'round'}));
    const tooltip=make('p','Pase el cursor o use Tab sobre un punto para ver el valor.','dash-chart-caption');
    items.forEach((d,i)=>{const dot=node('circle',{cx:x(i),cy:y(d.count),r:items.length>80?2:5,fill:'#fff',stroke:'#297f96','stroke-width':2,tabindex:0,role:'img','aria-label':`${d.label}: ${d.count} registros`});dot.append(node('title',{},`${d.label} · ${fmt(d.count)} registros`));const show=()=>tooltip.textContent=`${d.label}: ${fmt(d.count)} registros · ${pct(d.count,filtered.length)} del total filtrado`;dot.addEventListener('mouseenter',show);dot.addEventListener('focus',show);svg.append(dot);if(i===0||i===items.length-1||i%Math.max(1,Math.ceil(items.length/6))===0)svg.append(node('text',{x:x(i),y:218,'text-anchor':'middle',fill:'#718494','font-size':11},d.label));});host.append(svg,tooltip);
  ...
  async function drawMap(){const version=++mapTurn,total=filtered.length,groups=M.group(filtered,r=>M.geo(r).department),counts=new Map(groups.map(x=>[M.norm(x.label),x.count]));
    try{
      ...
      if(!geometry){const response=await fetch('assets/maps/departamentos.geojson');if(!response.ok)throw Error('No se pudo cargar la cartografía.');geometry=await response.json();}
      ...
      const color=n=>!n?'#e8edf1':n/Math.max(total,1)<.05?'#c9e2eb':n/total<.2?'#72b3c6':n/total<.5?'#337d95':'#164a61';
