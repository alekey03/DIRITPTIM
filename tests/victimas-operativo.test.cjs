...
(async()=>{
 ...
 const valid={apellido_paterno:'FICTICIO',nombres:'PRUEBA',edad:17,condicion_edad:'MENOR',fecha:'2026-09-15',situacion:'VICTIMA RESCATADA',entidad_disposicion:'UPE',genero:'MASCULINO',nacionalidad:'PERU',tipo_documento:'DNI'};
 const save=async(n,v=0,data=valid,parent=100)=>(await db.query('select guardar_victima_operativo($1,$2,$3,$4,$5) r',[id(n),id(parent),v,'victima',JSON.stringify(data)])).rows[0].r;
 ...
 for(const field of ['apellido_paterno','nombres','edad','fecha','situacion','condicion_edad']){const d={...valid};delete d[field];await assert.rejects(save(201,0,d));}
 for(const data of [{fecha:'2026-02-30'},{fecha:'abc'},{hora:'25:00'},{situacion:'inventada'},{edad:18},{condicion_edad:'MAYOR'},{genero:'inventado'},{tipo_documento:'inventado'},{nacionalidad:'inventada'},{foto:'imagen'},{unidad:'B'}])await assert.rejects(save(201,0,{...valid,...data}));
 for(const [n,age] of [[201,2],[202,18],[203,100]])await save(n,0,{...valid,edad:age,condicion_edad:age<18?'MENOR':'MAYOR'});
 const updated={...valid,situacion:'PRESUNTA VICTIMA',entidad_disposicion:'UDAVIT'};const second=await save(200,1,updated);assert.equal(second.version,2);assert.deepEqual(await save(200,1,updated),second);await assert.rejects(save(200,1,valid));
 ...
 await actor(2);assert.equal((await db.query('select * from intervencion_victimas')).rows.length,0);await assert.rejects(save(210));
 await actor(5);assert.equal((await db.query('select * from intervencion_victimas')).rows.length,4);await assert.rejects(save(210));await assert.rejects(save(200,2));
 await actor(4);assert.equal((await db.query('select * from intervencion_victimas')).rows.length,0);await assert.rejects(save(210));
 await actor(null);await assert.rejects(save(210));
 await actor(3);await save(200,2,valid);await assert.rejects(db.exec('delete from intervencion_victimas'));
 await db.query("insert into intervenciones(id,tipo) values($1,'operativo')",[id(101)]);
 await assert.rejects(db.query('update intervencion_victimas set intervencion_id=$1 where id=$2',[id(101),id(200)]));
 await assert.rejects(db.query('update intervencion_victimas set creado_por=$1 where id=$2',[id(3),id(200)]));
 await db.exec('reset role');await db.exec(migration);await db.exec('set role anon');await assert.rejects(db.exec('select * from intervencion_victimas'));
 await db.exec('reset role');assert.equal((await db.query('select count(*)::int n from intervencion_victimas')).rows[0].n,4);
 console.log('OK: 23 columnas; edad/condición, fechas y catálogos; alta/edición, reintentos, conflictos, RLS, inmutabilidad y reaplicación.');
