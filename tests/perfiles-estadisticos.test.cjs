...
const read=n=>fs.readFileSync(path.join(source,'supabase/migrations',n),'utf8');
...
(async()=>{
 ...
 for(const file of fs.readdirSync(path.join(source,'supabase/migrations')).filter(n=>n.endsWith('.sql') && n>='202609140001' && n<'202609230019' && !n.includes('dependencias_institucionales') && !n.includes('016_bienes')).sort()){console.log('apply',file);await db.exec(read(file));}
 await db.exec(fs.readFileSync(path.join(stage,'supabase/migrations/202609230019_roles_estadisticos.sql'),'utf8'));
 await db.exec(fs.readFileSync(path.join(stage,'supabase/migrations/202609230020_permisos_estadisticos.sql'),'utf8'));
